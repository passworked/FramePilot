...
**普通用户直接可见：**控制预设、只读/连续模式、原生或 1/2–1/4 分频、可保存的写入许可、本地世界经验模式、人数补偿和只读校准。这些足以完成日常选择和安全启停。

**高级模式：**“分辨率调节范围与规则”、稳定窗口、冷却时间、本地经验等待/置信度/升档上限、低人数上调许可、单步调整、精确阶梯校准、便携策略导入/导出和手动分辨率。这些规则不会被程序自行修改；自动变化的只有 SteamVR 分辨率。

## 本地世界经验

FramePilot VR 会把已验证的 VRChat 分辨率按硬件指纹、世界 ID、人数档和刷新率分频保存在本机。再次进入相同上下文时有三种使用方式：

- **关闭**：继续学习，但不让历史数据参与建议。
- **仅建议**：显示历史目标，不自动写入。
- **自动应用**：仅在用户已经允许写入且使用连续自适应时应用；默认等待人数稳定 5 秒，置信度至少 60%。

精确人数档直接采用历史安全值；缺少精确档时优先在上下两个本地档案之间插值，否则按人数档距离进行经验偏移。默认“平衡”补偿对更高人数每档降低 5%，低人数不主动推高；高级模式可以允许低人数上调。向上应用默认限制单次最多 15%，应用后沿用现有 5 秒观察、立即回退和禁升保护。历史压力值始终作为上限。

本地世界经验位于：

```text
%LOCALAPPDATA%\SteamVRAdaptiveResolution\vrc-context-profiles.json
```

面板可以只忘记当前世界，或清空当前硬件下的全部世界经验；这些操作不会删除匿名负载采集记录。
...
- 严重压力会合并为一次 5%/10% 降档；每次降档写入后等待完整 5 秒恢复窗口，避免把 SteamVR 重建渲染目标时的短暂卡顿误判为新一轮压力。
